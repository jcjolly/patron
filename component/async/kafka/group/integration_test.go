...
func TestConsume(t *testing.T) {
	...
	go func() {
		...
		factory, err := New("test1", uuid.New().String(), []string{topic}, dockerKafka.Brokers(), kafka.DecoderJSON(),
...
func TestConsume_ClaimMessageError(t *testing.T) {
	...
	go func() {
		...
		factory, err := New("test1", uuid.New().String(), []string{topic}, dockerKafka.Brokers(),
			kafka.Version(sarama.V2_1_0_0.String()), kafka.StartFromNewest())
		if err != nil {
			chErr <- err
			return
